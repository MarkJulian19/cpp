#include <stdio.h>
#include <math.h>
int mass(int M, int N, int *arr){//будем представлять матрицу в виде прямоугольников которые входят друг в друга и будем обходить каждый прямоугольник со сдвигом относительно внешнего по часовой стрелке до тех пор пока мы не разместим все нужные элементы
	int i = 0,j = 0, Ibeg = 0, Jbeg = 0, Ifin = 0, Jfin = 0,k = 1,s = M*N;//ввод нужных переменных
	while (k<=s){
		arr[i*N+j] = k;
